...
export default async function BlogDetailPage({ params }: BlogPageProps) {
  ...
  return <main className={styles.page}>
    ...
    <article>
      <header className={styles.articleHeader}><p className={styles.breadcrumb}><span>{post.blogCategory === "spaceMaking" ? "Space Making" : "Spacelabs"}</span><span aria-hidden="true">/</span><span>Home Decor</span><span aria-hidden="true">/</span><strong>{post.category}</strong></p><h1>{post.title}</h1><time>{post.date}</time><div className={styles.heroImage}><Image src={post.imageUrl} alt={post.imageAlt} fill priority sizes="100vw" /></div></header>
