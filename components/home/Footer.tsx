  ...
  return (
    <footer className="footer dark-section" id="clients">
      <div className="footer__grid section-shell">
        <div className="footer__lead"><p className="eyebrow">({content.eyebrow})</p><h2>{content.title.split("\n").map((line) => <span key={line}>{line}</span>)}</h2><a href={`mailto:${content.email}`}>↳&nbsp; {content.email}</a></div>
